...
- **导航器** —— 全部 opencode 会话按项目目录分组（目录可折叠）；分叉嵌套在父会话之下；点 ★ 收藏一条会话，收藏跨项目集中在侧栏顶部的「★ 收藏」区，画布只画当前选中的故事，不受收藏影响。右键可重命名（走 `PATCH /session`）或归档；右键目录可归档整个项目。归档项从所有视图隐藏（搜索、命令面板、画布），在侧栏底部归档区一键恢复——目录归档后新增的会话也保持隐藏，直到目录恢复。
