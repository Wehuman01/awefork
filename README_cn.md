...
- **增强搜索** —— 搜索框右侧的 ✦ 打开范围勾选（标题 / 标签 / 正文）和项目下拉（默认跨所有项目，可限定到单个项目）；同样的能力也能直接敲语法：`body:词` 扫会话正文（结果异步补上，大小写不敏感，命中片段高亮，点击定位到画布上的对应回合）、`#标签`、`title:`、`dir:`、`is:收藏` / `is:fork` / `is:运行中`、`"精确短语"`、`-词` 排除。全角冒号、前缀后带空格都能识别。
