...
- **Enhanced search** — the ✦ button next to the search box opens scope checkboxes (title / tags / body) and modifier chips; the same powers live in the box as syntax: `body:word` scans conversation bodies (results stream in with highlighted snippets; click one to land on that turn on the canvas), `#tag`, `title:`, `dir:`, `is:starred` / `is:fork` / `is:running`, `"exact phrase"`, and `-word` to exclude. Full-width colons and a space after a prefix work too.
