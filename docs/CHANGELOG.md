...
- **Model quick access (常用 / 最近)** — every model picker (pane composer, fork draft, compare sides) shows your high-frequency models first instead of making you search the catalog each time: ★ any row pins it into a 常用 group, hand-picks feed an automatic most-recent-first 最近 group, and the full catalog stays below with search unchanged. Both lists persist per backend in the composer sidecar; keyboard navigation walks 默认模型 → quick groups → catalog, and the highlight follows its model when starring reorders the rows.
